...
from veeduria_analysis import render_veeduria_tab
...
def soql_get(params: dict, dataset="contratos") -> pd.DataFrame:
    """Ejecuta query SoQL. Sin SQLite. Sin reintentos agresivos."""
    import time
    
    url = "https://www.datos.gov.co/resource/p6dx-8zbt.json" if dataset == "procesos" else API_BASE
...
def main():
    ...
    # ── Tabs de Navegación ────────────────────────────────────────────────────
    tab_geo, tab_red, tab_veeduria = st.tabs(["🌍 Explorador Territorial", "🕸️ Análisis de Redes & Anomalías", "👁️ Veeduría Ciudadana"])
    ...
    with tab_veeduria:
        render_veeduria_tab(anio, soql_get, normalizar, GEOJSON)
